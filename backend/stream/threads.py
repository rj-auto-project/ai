...
def thread_track(track_id, cx, cy):
    illegal_parking = threading.Thread(target=check_illegal_parking, args=(track_id,cx,cy))
    traffic_violation = threading.Thread(target=detect_traffic_violations, args=(track_id,cx,cy))
    ...
    illegal_parking.join()
    traffic_violation.join()
    wrong_way_driving.join()
...
def stream_process(camera_id, camera_ip, video_path, batch_size=8):
    ...
    out = cv2.VideoWriter("C:/Users/Laptop/Desktop/ai/output/output.mp4", fourcc, fps, (640,480))
    if not cap.isOpened():
