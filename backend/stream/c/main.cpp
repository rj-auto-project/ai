...
void stream_process(int camera_id, const string &camera_ip, const string &video_path, int batch_size) {
    ...
    VideoWriter out("output.mp4", VideoWriter::fourcc('m', 'p', '4', 'v'), fps, Size(width, height));
...
int main() {
    ...
    try {
        model = torch::jit::load("/home/annone/ai/models/objseg50e.pt");
    ...
    stream_process(camera_id, camera_ip, "/home/annone/ai/data/T-pole wrong way.mp4", 2);
